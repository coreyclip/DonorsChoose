...
for c_i, c in tqdm(enumerate(cols)):
    ...
    tfidf.fit(df_all[c])
    tfidf_train = np.array(tfidf.transform(train[c]).toarray(), dtype=np.float)
    tfidf_test = np.array(tfidf.transform(test[c]).toarray(), dtype=np.float)
...
for train_index, valid_index in kf.split(X):
    ...
    if len(p_buf) == 0:
        p_buf = np.array(p, dtype=np.float)
    else:
        p_buf += np.array(p, dtype=np.float)
    auc_buf.append(auc)
